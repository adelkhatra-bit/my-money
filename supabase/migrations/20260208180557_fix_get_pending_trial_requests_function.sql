/*
  # Fix Get Pending Trial Requests Function

  ## Changes
  - Drop and recreate the get_pending_trial_requests function with correct column names
  - Return proper data structure with all needed fields
*/
...
BEGIN
  ...
  SELECT 
    ...
    ftr.requested_at,
    (SELECT COUNT(*) FROM free_trial_requests WHERE user_id = up.user_id) as trial_count
  FROM free_trial_requests ftr
  INNER JOIN user_profiles up ON up.user_id = ftr.user_id
